...
from .dataset import CustomDistributionSampler
...
def chi_square_value(p, v, reg):
    ...
    with torch.no_grad():
        chi2 = (
            ...
            * (
                torch.norm(
                    m * p - torch.ones(m, device=p.device),
                    p=2,
                )
                ** 2
            )
...
class RobustLoss(nn.Module):
    ...
    def best_response(self, v):
        ...
        if self.geometry == "cvar":
            if self.reg > 0:
                ...
                def p(eta):
                    x = (v - eta) / reg
                    return (
                        torch.min(torch.exp(x), torch.Tensor([1 / size]).type(x.dtype))
                        / m
                    )
            ...
            if size == float("inf"):
                ...
                if reg == 0:
                    ...
                    def p(eta):
                        ...
                        opt_lam = max(reg, torch.norm(pp) / np.sqrt(m * (1 + 2 * size)))
        ...
        eta_star = bisection(
            eta_min, eta_max, bisection_target, tol=self.tol, max_iter=self.max_iter
        )
    ...
    def forward(self, v):
        ...
        if self.geometry == "cvar":
            ...
            if self.size == float("inf"):
                return (0.5 / self.reg) * (w**2).mean() + 0.5 * self.reg + self.eta
            else:
                if self.reg == 0:
                    return self.eta + np.sqrt((1 + 2 * self.size) / n) * torch.norm(
                        w, p=2
                    )
                else:
                    return (
...
class PrimalDualRobustLoss(nn.Module):
    ...
    def __init__(
        ...
        size,
        geometry,
        sampler: CustomDistributionSampler,
        step_size=1e-3,
        clip=0.01,
    ):
        super().__init__()
    ...
    def forward(self, v, update=True):
        if update and not self.is_erm:
            ...
            # pdb.set_trace()
            np.add.at(p_update, self.sampler.inds, v.detach().cpu().numpy() * coefs)
            #             pdb.set_trace()
...
class MultiLevelRobustLoss(torch.nn.Module):
    ...
    def forward(self, v):
        ...
        delta = self.robust_loss(v) - 0.5 * (
            self.robust_loss(v[: n // 2]) + self.robust_loss(v[n // 2 :])
        )
