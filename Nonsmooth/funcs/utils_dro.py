...
def aggregate_by_group(v, g, n):
    assert g.max() < n
    return csc_matrix((v, (g, np.zeros_like(g))), shape=(n, 1)).toarray().squeeze()


def subsample_arrays(arrays, n, seed=0):
    rng_state = np.random.get_state()
    np.random.seed(seed)
    take_inds = np.random.choice(len(arrays[0]), n, replace=False)
...
def average_step(model, model_avg, step, eta=0.0):
...
def average_step_ema(model, model_avg, gamma=0.9):
    ...
    for k in keys:
        model_avg.state_dict()[k].mul_(1 - gamma).add_(model.state_dict()[k].mul(gamma))
    return model_avg
...
def analytical_dro_objective(
    p, invcdf, size=1.0, geometry="cvar", reg=0.0, output_lambda=False
):
    if geometry == "cvar":
        assert reg == 0.0
        opt_lambda = None
        ...
        out = (1 / (p[-1] - 1 + size)) * np.trapz(
            np.concatenate([[var], invcdf[ind:]]), np.concatenate([[1 - size], p[ind:]])
        )
...
def project_to_cs_ball(v, rho):
    ...
    eta_simplex = optimize.brentq(target_simplex, eta_min_simplex, eta_max_simplex)
    ...
    eta_cs = optimize.brentq(target_cs, eta_min_cs, eta_max_cs)
...
def project_to_cvar_ball(w, alpha):
    ...
    if target_cvar(0.0) <= 0.0:
        ...
        nu_max = 0.0
        nu_min = logw.min()
        nu = optimize.brentq(target_cvar, nu_min, nu_max)
        p = target_cvar(nu, return_p=True)
    if p.max() > 1 / k * 1.01:
        logging.warning(
            f"project_to_cvar_ball: Maximum element of p"
            f" is {p.max()}; supposed to be {1/k}"
        )
    if np.abs(p.sum() - 1.0) > 1e-2:
        logging.warning(
            f"project_to_cvar_ball: Elements of p sum to" f"{p.sum()} instead of 1.0"
        )
    return p
...
def subsample_cvar_w(n, s, alpha):
    ...
    log_choices = binomln(i_s - 1, k_s) + binomln(n - i_s, s - k_s - 1) - binomln(n, s)
