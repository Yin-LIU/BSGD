...
class GroupedDataset(Dataset):
    def __init__(self, dataset: TensorDataset, group_ids, group_names):
        ...
        self.group_sizes = utils_dro.aggregate_by_group(
            np.ones_like(group_ids), group_ids, len(group_names)
        ).astype("int")
        ...
        if len(group_names) < 100:
            ...
            for i in range(len(group_names)):
                group_hist = np.unique(
                    labels[np.array(group_ids) == i], return_counts=True
                )
                # logging.info(
                #     f"Label breakdown for group {i}={group_names[i]}: "
                #     f"{tuple(zip(*group_hist))}; "
                #     f"total={self.group_sizes[i]}"
                # )

    def __getitem__(self, index):
        return tuple(self.dataset[index]) + (self.group_ids[index], index)
...
class MNISTandTypedFeatures(GroupedDataset):
    ...
    def __init__(
        ...
        if group_by_class:
            group_names = [f"{s}_{d}" for s in ("mnist", "typed") for d in range(10)]
        else:
        ...
        for i, name in enumerate(["MNIST", "typed_digits"]):
            ...
            #     root, name, features_name + "_features", split + ".pt"
            # )
            features_path = os.path.join(root, name + "_" + split + ".pt")
            data = torch.load(features_path)
            xy = data["features"], data["targets"]
            if subsample_to[i] > 0:
                xy = utils_dro.subsample_arrays(xy, subsample_to[i], subsample_seed + i)
...
class RandomBatchSizeSampler(Sampler):
    ...
    def __init__(
        ...
        self.batch_size_max = batch_size_max if batch_size_max > 0 else len(dataset)
            ...
            while i < self.n and batch_counter < self.num_batches:
                ...
                if i + batch_size <= len(inds):
                    ...
                    yield np.random.choice(
                        self.n, replace=self.replace, size=batch_size
                    )
                batch_counter += 1
                i += batch_size
    ...
    def sample_batch_size(self):
        ...
        return np.minimum(self.batch_size_max, self.batch_size_min * multiplier)

    def batch_size_pmf(self, bs):
        factor = np.ceil(np.log2(bs / self.batch_size_min))
        if bs < self.batch_size_max:
            p = (1 - self.doubling_probability) ** factor * self.doubling_probability
        else:
...
class CustomDistributionSampler(Sampler):
    ...
    def __init__(self, dataset, batch_size, replace=True, num_batches=None):
        self.n = len(dataset)
        self.num_batches = (
            num_batches if num_batches is not None else self.n // batch_size
        )
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            ...
            self.inds = np.random.choice(
                self.n, replace=self.replace, size=self.batch_size, p=p
            )
            yield self.inds
